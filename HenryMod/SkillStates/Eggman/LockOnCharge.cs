...
namespace HenryMod.SkillStates
{
	...
	public class LockOnCharge : BaseState
	{
		// Token: 0x0600132B RID: 4907 RVA: 0x00055494 File Offset: 0x00053694
		public override void OnEnter()
		{
			base.OnEnter();
			this.minChargeDuration = LockOnCharge.baseMinChargeDuration / this.attackSpeedStat;
			this.chargeDuration = LockOnCharge.baseChargeDuration / this.attackSpeedStat;
			base.PlayCrossfade("Gesture, Override", "LockOnCharge", "LockOnCharge.playbackRate", this.chargeDuration, 0.1f);
			base.PlayCrossfade("Gesture, Additive", "LockOnCharge", "LockOnCharge.playbackRate", this.chargeDuration, 0.1f);
			this.muzzleTransform = base.FindModelChild(LockOnCharge.muzzleName);
			if (this.muzzleTransform)
			{
				this.chargeupVfxGameObject = UnityEngine.Object.Instantiate<GameObject>(LockOnCharge.chargeupVfxPrefab, this.muzzleTransform);
				this.chargeupVfxGameObject.GetComponent<ScaleParticleSystemDuration>().newDuration = this.chargeDuration;
			}
			this.enterSoundID = Util.PlayAttackSpeedSound(LockOnCharge.enterSoundString, base.gameObject, this.attackSpeedStat);
			Util.PlaySound(LockOnCharge.playChargeSoundString, base.gameObject);
		}
		...
		public override void OnExit()
		{
			...
			Util.PlaySound(LockOnCharge.stopChargeSoundString, base.gameObject);
			base.OnExit();
		}
		...
		public override void FixedUpdate()
		{
			...
			if (base.fixedAge >= this.chargeDuration)
			{
				...
				{
					this.holdChargeVfxGameObject = UnityEngine.Object.Instantiate<GameObject>(LockOnCharge.holdChargeVfxPrefab, this.muzzleTransform);
				}
			}
			if (base.isAuthority)
			{
				...
				if (this.released)
				{
					this.outer.SetNextState(new LockOnFire());
				}
			}
		}
