...
namespace TechnoEmperorMod.SkillStates
{
	public class LockOn : BaseLockOnMissileState
	{
		public override void OnEnter()
		{
			...
			{
				this.crosshairOverrideRequest = CrosshairUtils.RequestOverrideForBody(base.characterBody, Modules.Assets.LoadCrosshair("Railgunner"), CrosshairUtils.OverridePriority.Skill);
			}
		...
		{
			if (this.targetsList.Contains(hurtBox))
			{
				return;
			}
		...
		private void CleanTargetsList()
		{
			...
			for (int j = this.targetsList.Count - 1; j >= 100; j--)
			{
		...
		private void AuthorityFixedUpdate()
		{
			...
			if (hurtBox != this.previousHighlightTargetHurtBox)
			{
				...
				this.stickyTargetIndicator.targetTransform = ((hurtBox) ? hurtBox.transform : null);
				this.stackStopwatch = 0f;
			}
